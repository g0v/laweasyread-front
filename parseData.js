...
function parseData(mojData, aliases) {
    const ret = mojData.map(law => ({PCode: law.PCode || law.pcode, name: law.name}));
    ...
    for(let PCode in aliases) {
        const law = mojData.find(law => law.PCode === PCode || law.pcode === PCode);
        if(typeof law === "undefined") {
            console.error(`PCode: ${PCode} not found`);
            continue;
        }
        aliases[PCode].forEach(name =>
            ret.push({PCode, name, fullName: law.name})
        );
    }
 ...
 * 由另一專案讀取所有法規資料
 * @see {@link https://github.com/kong0107/mojLawSplitJSON/tree/arranged }
 */
if(typeof module !== 'undefined' && module.exports) {
    const fs = require("fs");
    const mojData = JSON.parse(fs.readFileSync("../mojLawSplit/json_arrange/index.json").toString());
